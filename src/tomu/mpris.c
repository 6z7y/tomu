...
static void append_metadata(DBusMessageIter *var) {
  ...
  if (tctx.state.metadata.composer[0])
    dict_add_strarray1(&arr, "xesam:composer", tctx.state.metadata.composer);

  if (tctx.state.metadata.track[0])
    dict_add_int32(&arr, "xesam:trackNumber", (int32_t)atoi(tctx.state.metadata.track));

  if (tctx.state.metadata.disc[0])
    dict_add_int32(&arr, "xesam:discNumber", atoi(tctx.state.metadata.disc));
...
static void handle_set(DBusMessage *msg) {
  ...
  if (!strcmp(prop, "Volume")) {
    ...
    if (!strcmp(val, "None"))
      ...
      tctx.state.loop = LOOP_TRACK;
    else if (!strcmp(val, "Playlist"))
      tctx.state.loop = LOOP_PLAYLIST;
    pthread_mutex_unlock(&tctx.state.lock);
  }
...
static void handle_message(DBusMessage *msg) {
  if (dbus_message_is_method_call(msg, IFACE_PROPS, "Get")) {
    ...
    DBusError err;
    dbus_error_init(&err);
    if (dbus_message_get_args(msg, &err, DBUS_TYPE_STRING, &src, DBUS_TYPE_INVALID)) {
      reply_empty(msg);
...
void *mpris_loop(void *arg)
{
  ...
  while (T_RUE) {
    ...
    int running;
    pthread_mutex_lock(&tctx.state.lock);
    running = tctx.state.running;
    pthread_mutex_unlock(&tctx.state.lock);
    if (running) mpris_notify_change();
    sleep_ms(100);
  }
}
