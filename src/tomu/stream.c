...
int streaming_start(PlayBackContext *ctx, const char *url) {
  streaming_cleanup(&tctx.stream_ctx);   // clean up any prior stream first
  ...
  if (!ca) { streaming_cleanup(&tctx.stream_ctx); return -1; }

  ca->url = stream_url;
  ca->sb = sb;

  pthread_create(&ctx->stream_ctx.stream_thread, NULL, curl_thread_fn, ca);
  pthread_detach(tctx.stream_ctx.stream_thread);
...
int streaming_init_playback(PlayBackContext *ctx, const char *url) {
  ...
  fetch_url_metadata(url, &tctx.state.metadata);
  ...
  ctx->fmtCTX->max_analyze_duration = 3 * AV_TIME_BASE;

  if (avformat_open_input(&ctx->fmtCTX, NULL, NULL, NULL) < 0) {
    ctx->fmtCTX = NULL; // avformat_open_input already freed it on failure
  ...
  int stream_idx = av_find_best_stream(ctx->fmtCTX, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);
  if (stream_idx < 0) {
  ...
  AVStream *st = ctx->fmtCTX->streams[stream_idx];
